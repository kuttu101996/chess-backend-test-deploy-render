...
//       if (!challengedSocket) {
//         console.log(
//           `Challenge failed: User ${challengedUserId} is not online.`
//         );
...
//       console.log(
//         `User ${socket.user.name} challenged ${challengedUserId}. Game ID: ${game._id}`
//       );
//     } catch (error) {
...
//       const playerIds = [
//         populatedGame.whitePlayer._id,
//         populatedGame.blackPlayer._id,
//       ];
...
io.on("connection", (socket: SocketWithAuth) => {
  ...
  socket.on("find_match", async ({ userId, timeOption, rating }) => {
    if (!userId || !timeOption || typeof rating !== "number") {
    ...
    // Try to find a match
    const match = findMatch(timeOption, rating);
    if (match) {
      // Found a match, create a game and notify both players
      try {
        // Randomize colors
        const whitePlayer = Math.random() < 0.5 ? userId : match.userId;
        const blackPlayer = whitePlayer === userId ? match.userId : userId;
        const game = await Game.create({
          whitePlayer,
          blackPlayer,
        });
        ...
        match.socket.emit("match_found", {
          ...payload,
          opponentId: userId,
          color: whitePlayer === match.userId ? "white" : "black",
        });
        console.log(
          `Match found: Game ${game._id} (${whitePlayer} vs ${blackPlayer})`
        );
      } catch (err) {
        socket.emit("matchmaking_error", { message: "Failed to create game." });
        match.socket.emit("matchmaking_error", {
          message: "Failed to create game.",
        });
        console.error("Error creating game for matchmaking:", err);
      }
    } else {
      // No match found, add to pool and set timeout
      ...
      const added = addToPool(timeOption, entry);
      if (!added) {
        socket.emit("matchmaking_error", {
          message: "Already searching for a match.",
        });
      } else {
        socket.emit("searching", { message: "Searching for an opponent..." });
      }
    }
  });

  socket.on("cancel_match", ({ userId, timeOption }) => {
    if (!userId || !timeOption) return;
    removeFromPool(timeOption, userId);
